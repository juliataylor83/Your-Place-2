...
jQuery( document ).ready(function($) {
		...
		if ( quiz.currentQuestion < quiz.questionCount ) {
			...
			for (var i = 0; i<shuffled_answers.length; i++) {
				...
				if ( shuffled_answers[i].img !== '' || shuffled_answers[i].answer !== '' ) {
					...
					if ( quiz.quiz_settings.quiz_type == 'mc' && shuffled_answers[i].answer == answer.answer &&  shuffled_answers[i].img == answer.img ) {
	...
	function set_result( quiz ) {
		...
		if ( quiz.quiz_settings.quiz_type == 'pt' ) {
			...
			yourResult = winners[Math.floor(Math.random() * winners.length)]
		...
		//SEND RESULT
		if ( yourResult.hasOwnProperty('id') ) {
			add_result ( quiz.ajaxurl, quiz.nonce, quiz.quiz_id, yourResult.id )
		} else {
			add_result ( quiz.ajaxurl, quiz.nonce, quiz.quiz_id, quiz.score )
		}
		
		//RETURN WHAT THE RESULT IS FOR SOCIAL SHARING
		if ( quiz.quiz_settings.quiz_type == 'pt' ) {
			return yourResult.title
		} else {
			if ( yourResult.title ) {
				return quiz.score + '/' + quiz.questionCount + ': ' + yourResult.title
			} else {
				return quiz.score + '/' + quiz.questionCount
			}
		}
	}
	...
	function endTest( quiz ) {
		...
		add_activity( quiz.ajaxurl, quiz.nonce, quiz.quiz_id, 'completions' )
	...
	function show_optins ( quiz ) {
		...
		$( quiz.selector ).find( '.fca_qc_optin_input' ).tooltipster( {trigger: 'custom', maxWidth: 240, theme: ['tooltipster-borderless', 'tooltipster-quiz-cat'] } ).tooltipster('close')
	...
	var optins = []
	function add_to_mailing_list( ajaxurl, post_id, nonce, user_email, user_name, result ) {
		if ( optins.indexOf( post_id ) !== -1 ) {
	...
	var results = []
	function add_result ( ajaxurl, nonce, post_id, result ) {
		if ( results.indexOf( post_id ) !== -1 ) {
			//already got a result for this quiz
			console.log ( 'already stored results for this quiz! woop' )
			return false
		} else {
	...
	function encode_share_link ( $link, result ) {
		var string = encodeURIComponent ( $link.data('sharestring').replace('{{MY_QUIZ_RESULT}}', result ) )
