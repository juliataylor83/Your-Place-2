...
if ( !defined ('FCA_QC_PLUGIN_DIR') ) {
	...
	function fca_qc_admin_cpt_script( $hook ) {
		...
		if ( ($hook == 'post-new.php' || $hook == 'post.php')  &&  $post->post_type === 'fca_qc_quiz' ) {  
			...
			wp_enqueue_script('fca_qc_admin_js', FCA_QC_PLUGINS_URL . '/assets/admin/js/admin.min.js', array('jquery','jquery-ui-core', 'jquery-ui-tooltip'), FCA_QC_PLUGIN_VER, true );		
	...
	function fca_qc_render_description_meta_box( $post ) {
		...
		echo "<input type='hidden' name='fca_qc_quiz_preview_url' id='fca_qc_quiz_preview_url' value='" . get_permalink( $post ) . "'>";
		
		echo "<label class='fca_qc_admin_label'>" . __('Description', 'quiz-cat') . "</label>";

		echo "<textarea class='fca_qc_texta' placeholder='". __('Description', 'quiz-cat') . "' id='fca_qc_quiz_description' name='fca_qc_quiz_description'>" . $quiz_meta['desc'] . "</textarea>";	
			...
			$html .= "<h3 class='fca_qc_question_label'><span class='fca_qc_quiz_heading_question_number'>" . __('Question', 'quiz-cat') . ' ' . $question_number . ": </span><span class='fca_qc_quiz_heading_text'>". fca_qc_convert_entities($question['question']) . "</span></h3>";
				...
				$html .= "<div class='fca_qc_question_input_div'>";
					$html .= "<div class='fca_qc_question_header_div'>";
						$html .= fca_qc_add_image_input( $question['img'], "fca_qc_quiz_questions[$question_number][img]" );
						$html .= "<textarea placeholder='" . __('e.g. Can cats fly?', 'quiz-cat') . "' class='fca_qc_question_texta fca_qc_question_text' name='fca_qc_quiz_questions[$question_number][question]'>" . $question['question']  ."</textarea><br>";				
					$html .= "</div>";
	...
	function fca_qc_render_answer( $answer, $question_number, $answer_number ) {
		...
		$html .= "<div class='fca_qc_answer_input_div fca_qc_deletable_item'>";
			...
			if ( $answer_number == 1 ) {
				...
				$html .= "<label class='fca_qc_admin_label'>" . __('Wrong Answer', 'quiz-cat') . " <a class='fca_qc_delete_button'>(" . __('remove', 'quiz-cat') . ")</a></label>";
			}
			if ( function_exists( 'fca_qc_save_premium_post' ) ) {
				$html .= fca_qc_add_image_input( $answer['img'], "fca_qc_quiz_questions[$question_number][answers][$answer_number][img]" );
			}
	...
	function fca_qc_render_result( $result, $result_number ) {
		...
		$html = "<div class='fca_qc_result_item fca_qc_deletable_item' id='fca_qc_result_$result_number'>";
			...
			$html .= "<div class='fca_qc_result_input_div'>";
					...
					$html .= "<input type='text' placeholder='" . __('Title', 'quiz-cat') . "' class='fca_qc_text_input fca_qc_quiz_result' name='fca_qc_quiz_result_title[]' value='" . $result['title'] . "'></input><br>";
					...
					$html .= "<textarea class='fca_qc_question_texta' placeholder='" . __('Description', 'quiz-cat') . "' name='fca_qc_quiz_result_description[]'>" . $result['desc'] . "</textarea><br>";
			$html .= '</div>';
	...
	function fca_qc_suppress_post_title() {
		...
		if ( $post->post_type == 'fca_qc_quiz' &&  is_main_query() ) {
			wp_enqueue_style( 'fca_qc_quiz_post_stylesheet', plugins_url( 'assets/admin/css/hide-title.css', __FILE__ ), array(), FCA_QC_PLUGIN_VER );
		}
	}	
	...
	function fca_qc_do_quiz( $atts ) {
		...
		if ( !empty ( $atts[ 'id' ] ) ) {
			...
			wp_enqueue_script( 'jquery' );
			wp_enqueue_style( 'fca_qc_quiz_stylesheet', plugins_url( 'assets/quiz/css/quiz.min.css', __FILE__ ), array(), FCA_QC_PLUGIN_VER );

			wp_enqueue_script( 'fca_qc_img_loaded', plugins_url( 'assets/quiz/js/jquery.waitforimages.min.js', __FILE__ ), array(), FCA_QC_PLUGIN_VER, true );
			wp_enqueue_script( 'fca_qc_quiz_js', plugins_url( 'assets/quiz/js/quiz.min.js', __FILE__ ), array('fca_qc_img_loaded'), FCA_QC_PLUGIN_VER, true );
		
			$optin_settings = get_post_meta ( $post_id, 'quiz_cat_optins', true );
			$restart_button = empty ( $quiz_settings['restart_button'] ) ? false : true;
			$draw_optins = empty( $optin_settings['capture_emails'] ) ? false : true;
			
			if ( $draw_optins ) {
				wp_enqueue_style( 'fca_qc_tooltipster_stylesheet', plugins_url( 'assets/quiz/css/tooltipster.bundle.min.css', __FILE__ ), array(), FCA_QC_PLUGIN_VER );
				wp_enqueue_style( 'fca_qc_tooltipster_borderless_css', plugins_url( 'assets/quiz/css/tooltipster-borderless.min.css', __FILE__ ), array(), FCA_QC_PLUGIN_VER );
				wp_enqueue_script( 'fca_qc_tooltipster_js', plugins_url( 'assets/quiz/js/tooltipster.bundle.min.js', __FILE__ ), array('jquery'), FCA_QC_PLUGIN_VER, true );
			}
